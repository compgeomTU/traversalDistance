...
class FreeSpaceGraph:
    def __init__(self, g1, g2, epsilon, log=False):
        ...
        # diables logging
        if not log:
            self.traversal_logger.propagate = False
            self.dfs_logger.propagate = False
            self.path_logger.propagate = False
            self.projection_logger.propagate = False
 ...
 (right vertex) .________________
    ...
    def DFS(self, cb, paths, curr_path): # f is written to a file, p is written for the path
        ...
        for neighbor in cb.g_verts.nodeLink[cb.vertexID]:
            ...
            self.dfs_logger.info("edge1=" + str(edge1) + "   edge2=" + str(edge2)+"\n")
            ...
            for V in [left_vertexID, right_vertexID]:
                ...
                if (cb.vertexID, neighbor) in cb.g_verts.edgeHash: 
                    ...
                    self.dfs_logger.info("start + end values: " + str(newCB.start_fs) + " " + str(newCB.end_fs)+"\n")
                    if newCB.visited == False and newCB.start_fs <= newCB.end_fs:
                        ...
                        self.path_logger.info("DFS -- basecase -> dont return path\n")
                        paths += [curr_path] 
            # end for thru L,R
            """ VERTICAL Boundary"""  
            ...
            self.dfs_logger.info("start + end values: " + str(newCB.start_fs) + " " + str(newCB.end_fs)+"\n")
            if newCB.visited == False and newCB.start_fs <= newCB.end_fs:
                self.dfs_logger.info("DFS -- add "+str(newCB.print_cellboundary())+"\n")
    ...
    def compute_union(self, intervals, mycb):
        ...
        self.traversal_logger.info("intervalsRETURN: "+str(intervalsRETURN))
